...
public class WhitelistCheck {
    ...
    static boolean isWhitelisted(@NonNull Context context, @NonNull String identifier) {
        try {
            if (isWhatsAppConsumerAppNotInstalled(context.getPackageManager()) && isWhatsAppSmbAppNotInstalled(context.getPackageManager())) {
                return false;
            }
    ...
    public static boolean isWhatsAppConsumerAppNotInstalled(PackageManager packageManager) {
        return !WhitelistCheck.isPackageInstalled(CONSUMER_WHATSAPP_PACKAGE_NAME, packageManager);
    }

    public static boolean isWhatsAppSmbAppNotInstalled(PackageManager packageManager) {
